...
instance.interceptors.request.use((config) => {
    ...
    // 5. 通过请求头携带token数据
    const token = store.state.token || localStorage.getItem('token_key')
    config.headers.versionCode = '1111'
    config.headers.phoneNum = '18406555698'
        // 有token就携带
    if (token) {
    ...
    if (config.headers['Access-Control-Expose-Headers'] === 'loginInAgain') {
